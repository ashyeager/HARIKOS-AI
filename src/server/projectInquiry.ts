...
export async function submitProjectInquiry(payload: unknown): Promise<InquiryResponse> {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return {
      status: 503,
      body: { error: "Inquiry submissions are temporarily unavailable. Please contact HARIKOS directly." },
    };
  }
  ...
  if (resendKey) {
    ...
    const safe = {
      name: escapeHtml(inquiry.full_name),
      email: escapeHtml(inquiry.email),
      company: escapeHtml(inquiry.company),
      industry: escapeHtml(inquiry.industry),
    ...
    const results = await Promise.allSettled([
      resend.emails.send({
        from,
        to: [adminEmail],
        subject: `New HARIKOS inquiry: ${inquiry.service}`,
        html: `<h2>New HARIKOS inquiry</h2><p><strong>Name:</strong> ${safe.name}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Company:</strong> ${safe.company}</p><p><strong>Industry:</strong> ${safe.industry}</p><p><strong>Interest:</strong> ${safe.service}</p><p><strong>Message:</strong><br />${safe.description}</p>`,
      }),
      resend.emails.send({
        from,
        to: [inquiry.email],
        subject: "We've received your HARIKOS message",
        html: `<p>Hello ${safe.name},</p><p>Thank you for contacting HARIKOS. We have received your message and will review it shortly.</p><p>You can also reach HARIKOS at <a href="mailto:ashyeagerhq@gmail.com">ashyeagerhq@gmail.com</a> or on Instagram at @harikos.ai.</p><p>HARIKOS</p>`,
      }),
    ]);

    for (const result of results) {
      if (result.status === "rejected") console.error("Inquiry email failed:", result.reason);
    }
  }
